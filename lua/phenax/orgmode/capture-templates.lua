...
function M.capture_templates(notes_path)
  return {
    d = {
      description = 'Daily',
      target = vim.fs.joinpath(notes_path, 'daily/%<%Y-%m>.org'),
      template = [=[
* %<%Y-%m-%d> %<%A> :daily:
...
-----
    ...
    W = {
      ...
      target = vim.fs.joinpath(notes_path, 'projects/work/%^{Project name}.org'),
      template = [[
* %? :work:
...
** Notes
    ...
    P = {
      ...
      target = vim.fs.joinpath(notes_path, 'projects/%^{Project name}.org'),
      template = [[
* %? :project:
...
** Notes
    ...
    N = {
      ...
      target = vim.fs.joinpath(notes_path, 'blackhole/logs.org'),
      template = [=[
* %T
